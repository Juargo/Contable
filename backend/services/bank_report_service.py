"""Servicio para procesar reportes bancarios de diferentes entidades financieras"""
...
def process_bank_report(file_content: bytes, bank_name: str) -> List[Dict[str, Any]]:
    ...
    if bank_name not in processors:
        raise ValueError(
            f"No hay un procesador implementado para el banco: {bank_name}"
        )
...
def process_santander_report(file_content: bytes) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        column_map = {"Fecha": "date", "Concepto": "description", "Importe": "amount"}

        # Renombrar columnas según el mapeo
        df = df.rename(
            columns={old: new for old, new in column_map.items() if old in df.columns}
        )
        ...
        transactions = df[["date", "description", "category", "amount"]].to_dict(
            "records"
        )
...
def process_bbva_report(file_content: bytes) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        column_map = {"F.VALOR": "date", "CONCEPTO": "description", "IMPORTE": "amount"}
...
def process_generic_report(file_content: bytes, bank_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for encoding in ["utf-8", "iso-8859-1", "latin1"]:
            try:
                ...
                date_cols = [
                    ...
                    if re.search(r"fecha|date|dia", col.lower())
                ]
                desc_cols = [
                    col
                    for col in df.columns
                    if re.search(r"concepto|desc|concept", col.lower())
                ]
                amount_cols = [
                    col
                    for col in df.columns
                    if re.search(r"importe|amount|valor|cantidad", col.lower())
                ]
        ...
        raise ValueError(
            f"No se pudo determinar automáticamente el formato del reporte del banco {bank_name}"
        )
        ...
        return [
            {
                "date": "2023-01-01",
                "description": f"Ejemplo {bank_name}",
                "category": "Sin clasificar",
                "amount": 0,
            },
            {
                "date": "2023-01-02",
                "description": "Se requiere implementación específica",
                "category": "Sin clasificar",
                "amount": 0,
            },
        ]
